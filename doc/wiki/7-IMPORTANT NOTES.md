* During the usage, sometimes the LED should turn on. This indicates that the device need to save a new configuration (this should happen especially when you try to get a new number from the relay server, or you format the "eeprom" and lost the Mobile Trainer configuration, or even during the Mobile Trainer first configuration). If this happens, **LET THE LED TURNS OFF BEFORE DO ANY OTHER ACTION IN GAME!** 
