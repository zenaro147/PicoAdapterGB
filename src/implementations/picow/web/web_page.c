...
"<label>Relay Number: <span id=\"relay_number\"></span></label>"
"</fieldset>"
...
"<label>Upload mobile_config.bin<input type=\"file\" id=\"eeprom_file\" accept=\".bin\"></label>"
"<label><input type=\"checkbox\" id=\"eeprom_replace_libmobile\"> Overwrite custom server config too? (like DNS and Relay Server)</label>"
"<button type=\"button\" id=\"eeprom_upload\">Upload mobile_config.bin</button>"
"<button type=\"button\" id=\"eeprom_dump\">Download mobile_config.bin</button>"
"</fieldset>"
...
"<div id=\"ver\"></div>"
"<script>"
"const DEVICE_LABELS={BLUE:'Blue (PDC)',YELLOW:'Yellow (cdmaOne)',GREEN:'Green (PHS-NTT)',RED:'Red (DDI)'};"
"function updateDeviceSummary(dev,unmetered){"
"device_summary.textContent='Current device: '+(DEVICE_LABELS[dev]||dev)+(unmetered?' - unmetered':'');"
"}"
"async function load(){"
...
"updateDeviceSummary(j.device,j.unmetered);"
...
"updateDeviceSummary(decoded.device,decoded.unmetered);"
"}"
"}"
...
"const a=document.createElement('a');a.href=url;a.download='mobile_config.bin';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);"
