...
"relayNumberPollCount=0;updateRelayNumberDisplay(j);"
...
"}"
"}"
"let relayNumberPollCount=0;"
"const RELAY_NUMBER_MAX_POLLS=10;"
"function updateRelayNumberDisplay(j){"
"if(!j.relay){relay_number.textContent='No relay server configured';return;}"
"if(j.relay_number){relay_number.textContent=j.relay_number;return;}"
"if(relayNumberPollCount>=RELAY_NUMBER_MAX_POLLS){relay_number.textContent='Not available (could not reach the relay server)';return;}"
"relay_number.textContent='(fetching...)';"
"relayNumberPollCount++;"
"setTimeout(pollRelayNumber,3000);"
"}"
...
"}catch(err){"
"console.error('[web] relay number poll failed',err);"
"}"
"}"
"function fields(){"
